...
// Scoring rules (from the sweepstake sheet):
//   win  = 3 pts
//   draw = 1 pt
//   loss = 0 pts
//   + 4 pts each time a team reaches a knockout round (R32, R16, QF, SF, Final)
//   + 4 pts for winning the World Cup
//   goal difference tracked as a tie-breaker / display stat
export const POINTS = { win: 3, draw: 1, loss: 0, nextRound: 4, champion: 4 }

// Stage codes from football-data.org. GROUP_STAGE earns no bonus; appearing in
// any of these stages means the team advanced -> +4. THIRD_PLACE is deliberately
// excluded: the sheet's rounds are R32/R16/QF/SF/F/W, so losing semi-finalists
// get nothing extra for the bronze match.
const BONUS_STAGES = new Set([
  "LAST_32",
  "LAST_16",
  "QUARTER_FINALS",
  "SEMI_FINALS",
...
export interface Match {
  ...
  winner?: "HOME" | "AWAY" | "DRAW" | null // needed when the final goes to pens
}
...
// Who lifted the trophy, or null while undecided. The final can be level after
// extra time (decided on pens), so trust the explicit winner field — set by
// both data sources — before falling back to comparing scores.
export function finalWinner(matches: Match[]): string | null {
  for (const m of matches) {
    if (m.stage !== "FINAL" || !FINISHED.has(m.status)) continue
...
export function computeTeamStats(matches: Match[]): Record<string, TeamStats> {
  ...
  for (const team of Object.keys(stats)) {
    ...
    if (team === champion) s.bonusPoints += POINTS.champion
