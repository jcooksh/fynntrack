...
export interface LiveOverride {
  ...
  minute?: string
  winner?: "HOME" | "AWAY" // who won a completed match (matters when the final goes to pens)
}
...
export async function fetchLiveOverrides(
  ...
  for (const event of data.events ?? []) {
    ...
    out.set(key, {
      ...
      // ESPN flags the victor on completed games (covers shootout wins, where
      // the score alone is level)
      winner:
        state === "post" && home.winner === true ? "HOME"
        : state === "post" && away.winner === true ? "AWAY"
        : undefined,
    })
  }
  return out
