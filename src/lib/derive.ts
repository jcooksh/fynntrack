...
import { finalWinner, isMatchLive, POINTS, type Match } from "@/lib/scoring"
...
export function buildTeamRows(matches: Match[]): TeamRow[] {
  ...
  for (const team of Object.keys(TEAM_OWNER)) {
    ...
    const champBonus = team === champion ? POINTS.champion : 0
    ...
    rows.push({
      ...
      points: won * 3 + drawn + koBonus + champBonus,
      form,
      stage: st,
